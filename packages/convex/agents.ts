...
export const updateAgentMetrics = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const newAvgResponseTime =
      (currentMetrics.avgResponseTime * currentMetrics.totalConversations +
        args.responseTime) /
      newTotal;
    ...
    if (args.satisfaction !== undefined) {
      newAvgSatisfaction =
        (currentMetrics.avgSatisfaction * currentMetrics.totalConversations +
          args.satisfaction) /
        newTotal;
    }
  ...
  handler: async (ctx, args) => {
    ...
    if (args.category) {
      ...
      filteredAgents = agents.filter(
        (a) =>
          (a.specialization?.categories &&
            a.specialization.categories.includes(category)) ||
    ...
    if (args.language) {
      ...
      filteredAgents = filteredAgents.filter(
        (a) =>
          (a.specialization?.languages &&
            a.specialization.languages.includes(language)) ||
