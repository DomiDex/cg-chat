...
export const createAgent = mutation({
    args: {
        name: v.string(),
        type: v.union(v.literal('chat'), v.literal('support'), v.literal('technical'), v.literal('sales'), v.literal('escalation')),
...
export const getAgents = query({
    args: {
        type: v.optional(v.union(v.literal('chat'), v.literal('support'), v.literal('technical'), v.literal('sales'), v.literal('escalation'))),
...
export const updateAgentMetrics = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const newAvgResponseTime = (currentMetrics.avgResponseTime * currentMetrics.totalConversations + args.responseTime) /
            newTotal;
        ...
        const escalationCount = Math.round((currentMetrics.escalationRate * currentMetrics.totalConversations) / 100);
...
export const getAvailableAgent = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.category) {
            ...
            filteredAgents = agents.filter((a) => (a.specialization?.categories && a.specialization.categories.includes(category)) ||
                !a.specialization?.categories?.length);
        }
        if (args.language) {
            const language = args.language;
            filteredAgents = filteredAgents.filter((a) => (a.specialization?.languages && a.specialization.languages.includes(language)) ||
