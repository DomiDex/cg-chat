...
export const getUserConversations = query({
    args: {
        userId: v.id('users'),
        status: v.optional(v.union(v.literal('active'), v.literal('archived'), v.literal('resolved'), v.literal('escalated'))),
    },
    handler: async (ctx, args) => {
        const query = ctx.db
            .query('conversations')
...
export const searchConversations = query({
    args: {
        ...
        status: v.optional(v.union(v.literal('active'), v.literal('archived'), v.literal('resolved'), v.literal('escalated'))),
        channel: v.optional(v.union(v.literal('web'), v.literal('whatsapp'), v.literal('email'), v.literal('api'))),
...
export const getActiveWhatsAppConversation = query({
    ...
    handler: async (ctx, args) => {
        const conversations = await ctx.db
            .query('conversations')
            .withIndex('by_user', (q) => q.eq('userId', args.userId))
            .filter((q) => q.and(q.eq(q.field('channel'), 'whatsapp'), q.eq(q.field('status'), 'active')))
            .order('desc')
            .first();
