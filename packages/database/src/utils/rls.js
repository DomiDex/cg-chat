 ...
 */
export async function createRLSClient(prisma, context) {
    ...
    ${context.sessionId ? `SET LOCAL app.current_session_id = '${context.sessionId}';` : ''}
  `);
    return prisma;
}
/**
 ...
 */
export async function withRLS(prisma, context, callback) {
...
export class RLSPool {
    ...
    async query(text, values, context) {
        ...
        try {
         ...
         SET LOCAL app.current_user_role = $2;`, [context.userId, context.userRole]);
 ...
 */
export function rlsMiddleware(defaultContext) {
 ...
 */
function getContextFromAsyncStorage() {
 ...
 */
export async function verifyRLSEnabled(prisma, tableName) {
 ...
 */
export async function listRLSPolicies(prisma, tableName) {
    const result = await prisma.$queryRaw `
    SELECT 
 ...
 */
export async function bypassRLS(prisma, callback) {
    return prisma.$transaction(async (tx) => {
 ...
 */
export async function testRLSAccess(prisma, context, tableName, operation) {
    try {
