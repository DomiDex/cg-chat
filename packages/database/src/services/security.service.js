...
export class SecurityService {
     ...
     */
    static async testUserAccess(userId, tests) {
        // Get user details
        const user = await prisma.user.findUnique({
            where: { id: userId },
            select: { id: true, role: true },
            ...
            for (const operation of operations) {
                try {
                    ...
                    await prismaRLS.$withRLS(context, async () => {
                        switch (operation) {
                            case 'SELECT':
                                const selectResult = await prisma.$queryRawUnsafe(`SELECT COUNT(*) FROM ${table}`);
                                allowed = true;
                                break;
                            case 'INSERT':
                            ...
                            case 'UPDATE':
                                ...
                                await prisma.$queryRawUnsafe(`UPDATE ${table} SET id = id WHERE false`);
                                allowed = true;
                                break;
                            case 'DELETE':
                                // Test delete without actually removing anything
                                await prisma.$queryRawUnsafe(`DELETE FROM ${table} WHERE false`);
                                allowed = true;
                                break;
                        }
                    });
     ...
     */
    static async filterByRLS(data, context) {
     ...
     */
    static createSecureQuery(context) {
        return {
            users: {
                findMany: async (args) => {
     ...
     */
    static async getUserSecurityAudit(userId, options) {
        const where = {
            userId,
